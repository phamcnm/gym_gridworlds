...
for init_value in [-10, 0, 10]:
    ...
    for i, gamma in enumerate(gammas):
        ...
        axs[0][i].imshow(tile_values)
        axs[1][i].plot(errors)
        axs[0][i].set_title(f'$\gamma$ = {gamma}')
        axs[1][i].xaxis.set_major_locator(MaxNLocator(integer=True, nbins='auto'))
        axs[1][i].tick_params(axis='x', labelsize=5)
        axs[1][i].tick_params(axis='y', labelsize=5)
    ...
    for i, gamma in enumerate(gammas):
        ...
        for a in range(n_actions):
            ...
            axs[a][i].imshow(tile_values)
            axs[a][i].set_xticks([])
            axs[a][i].set_yticks([])
        axs[-1][i].plot(errors)
        axs[0][i].set_title(f'$\gamma$ = {gamma}')
        axs[0][i].tick_params(axis='x', labelsize=6)
        axs[0][i].tick_params(axis='y', labelsize=6)
