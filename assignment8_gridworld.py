...
def collect_data(env, weights, eps, n_episodes):
    data = dict()
    data["phi"] = []
    data["a"] = []
    data["r"] = []
...
def dlog_softmax_probs(phi, weights, eps, act):
    ...
    mask = np.zeros((phi.shape[0], phi.shape[1], n_actions))
    for i in range(phi.shape[0]):
        mask[i, :, act[i, 0]] = 1
    first_term = phi_sa * mask

    probs = softmax_probs(phi, weights, eps)
    second_term = phi_sa * probs[:, None, :]
...
def reinforce(baseline="none"):
    ...
    while tot_steps < max_steps:
        # collect data
        data = collect_data(env, weights, eps, episodes_per_update)
        ...
        dlog = dlog_softmax_probs(phi, weights, sigma, A)
        # compute baseline
        b = 0
