...
def td(env, env_eval, Qs, gamma, eps, alpha, max_steps, alg):
    be = []
    exp_ret = []
    ...
    while True:
        ...
        while not done:
            if tot_steps % 100 == 0:
                greedy_val = expected_return(env_eval, (Qs[0] + Qs[1])/2, gamma)
                exp_ret.append(greedy_val)
                error = calc_be((Qs[0] + Qs[1])/2, gamma, eps, alg)
                be.append(error)
            a = eps_greedy_action((Qs[0] + Qs[1]), s, eps)
            s_next, r, terminated, truncated, _ = env.step(a)
            done = terminated or truncated
            target_value = 0
            which_Q = np.random.rand() < 0.5
            if not terminated:
                if alg == 'QL':
                    target_value = Qs[1-which_Q][s_next, np.argmax(Qs[which_Q][s_next])]
                elif alg == 'SARSA':
                    a_next = eps_greedy_action(Qs[which_Q], s_next, eps)
                    target_value = Qs[1-which_Q][s_next, a_next]
                elif alg == 'Exp_SARSA':
                    pi_s = eps_greedy_probs_state(Qs[which_Q], s_next, eps)
                    for a_next in range(len(pi_s)):
                        target_value += pi_s[a_next] * Qs[1-which_Q][s_next, a_next]
            td_error = r + gamma * target_value - Qs[which_Q][s, a]
            tde[tot_steps] = abs(td_error)
            Qs[which_Q][s, a] += alpha*td_error
...
for i, init_value in enumerate(init_values):
    ...
    for j, alg in enumerate(algs):
        ...
        for seed in seeds:
            np.random.seed(seed)
            Qs = [np.zeros((n_states, n_actions)) + init_value, np.zeros((n_states, n_actions)) + init_value]
            Q, be, tde, exp_ret = td(env, env_eval, Qs, gamma, eps, alpha, max_steps, alg)
